...
class InvoiceDetailsProcessor:
    ...
    def to_config(self, invoice: Invoice) -> Dict[str, Any]:
        return {
            ...
            'buyer': invoice.buyer,
            'invoice_items': [self.invoice_item_to_config(invoice.invoice_items[index], index + 1,
                                                          invoice.currency) for index in
                              range(len(invoice.invoice_items))],
            ...
            'total_invoice_value': self._price_formatter.format_price(
                sum([self._price_formatter.round_price(it.count_total_value()) for it in
